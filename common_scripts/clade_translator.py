...
def parse_args():
    ...
    parser = argparse.ArgumentParser(
        description="Annotate sequences using a genbank reference"
    )
    parser.add_argument(
        "--original-reference",
        help="Genbank sequence of the original reference",
    )
    parser.add_argument(
        "--new-reference", help="Genbank sequence of the new reference"
    )
    parser.add_argument("--clades", help="clades tsv")
    parser.add_argument(
        "--output-clades", required=True, type=str, help="Output file"
    )
...
def get_coordinate_map(ref, qry):
    ...
    with TemporaryDirectory() as tmp_dir:
        SeqIO.write(
            [
                SeqRecord.SeqRecord(id="ref", seq=Seq.Seq(ref)),
                SeqRecord.SeqRecord(id="qry", seq=Seq.Seq(qry)),
            ],
            f"{tmp_dir}/sequences.fasta",
            "fasta",
        )

        os.system(
            f"mafft --quiet {tmp_dir}/sequences.fasta > {tmp_dir}/aligned.fasta"
        )
        ref_aligned, qry_aligned = AlignIO.read(
            f"{tmp_dir}/aligned.fasta", "fasta"
        )
    ...
    return np.concatenate(
        [ref_to_qry, [min(ref_to_qry[-1] + 1, len(qry))]]
    ), np.concatenate([qry_to_ref, [min(qry_to_ref[-1] + 1, len(ref))]])
...
if __name__ == "__main__":
    ...
    orig_features = {
        ...
        for f in orig_ref.features
        if "gene" in f.qualifiers and f.type == "CDS"
    }
    new_features = {
        f.qualifiers["gene"][0]: f
        for f in new_ref.features
        if "gene" in f.qualifiers and f.type == "CDS"
    }

    coord_maps = {"nuc": get_coordinate_map(orig_ref.seq, new_ref.seq)}
    for f in orig_features:
        coord_maps[f] = get_coordinate_map(
            orig_features[f].extract(orig_ref).translate().seq,
            new_features[f].extract(new_ref).translate().seq,
        )

    df = pd.read_csv(args.clades, sep="\t", comment="#")

    with open(args.output_clades, "w") as f:
        f.write("clade\tgene\tsite\talt\n")
        for row in df.itertuples():
            print(row)
            clade = getattr(row, "clade")
            gene = getattr(row, "gene")
            site = getattr(row, "site")
            alt = getattr(row, "alt")
            if gene is np.nan:
                f.write("\t\t\t\n")
            if gene == "clade":
                f.write(f"{clade}\t{gene}\t{site}\t\n")
            elif gene in coord_maps:
                new_pos = max(0, coord_maps[gene][0][int(site) - 1]) + 1
                f.write("\t".join([clade, gene, str(new_pos), alt]) + "\n")
