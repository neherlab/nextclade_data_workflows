...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--ordering", type=str, required=True, help="input ordering file"
    )
    parser.add_argument(
        "--color-schemes",
        type=str,
    ...
    parser.add_argument(
        "--clade-node-data",
        type=str,
        help="if provided, restrict to only those clades found in tree",
    )
    parser.add_argument(
        "--output", type=str, required=True, help="output colors tsv"
    )
    ...
    if args.metadata:
        ...
        for name, trait in assignment.items():
            if name in metadata:
                subset_present = [
                    x for x in assignment[name] if x in metadata[name].unique()
                ]
                assignment[name] = subset_present
            if name in metadata and "focal" in metadata:
                focal_list = metadata.loc[
                    metadata["focal"] == True, name
                ].unique()
    ...
    if args.clade_node_data and "clade_membership" in assignment:
        ...
        assignment["clade_membership"] = [
            x for x in assignment["clade_membership"] if x in subset_present
        ]
    ...
    with open(args.output, "w") as f:
        for trait_name, trait_array in assignment.items():
            ...
            if len(schemes) < len(trait_array):
                print(
                    f"WARNING: insufficient colours available for trait {trait_name} - reusing colours!"
                )
            ...
            extra_color_values = list(
                forced_colors.get(trait_name, {}).values()
            )

            zipped = list(
                zip(
                    trait_array + extra_trait_values,
                    color_array + extra_color_values,
                )
            )
